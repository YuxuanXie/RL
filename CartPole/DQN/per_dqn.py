...
class Net(nn.Module):
    def __init__(self, state_space, action_space, hidden_size=64) -> None:
        super().__init__()
...
class DQN():
    def __init__(self, state_space, action_space, hidden_size=64) -> None:
        ...
        # Create model
        self.model = Net(state_space, action_space, hidden_size)
        self.model.apply(self.weights_init)
        self.target_model = Net(state_space, action_space, hidden_size)
    ...
    # inference
    def choose_action_to_env(self, s):
        state = torch.from_numpy(s)
        state = Variable(state).float().cpu()
        qs = self.model(state)
        _, action = torch.max(qs, -1)
...
if __name__ == '__main__':
    ...
    agent.load(os.path.dirname(os.path.abspath(__file__)) + f'/{env_name}.pth')

    for e in range(int(2e3)):
        done = False
        score = 0
        state = env.reset()
        ...
        if e % 500 == 0 and e != 0:
            torch.save(agent.model.state_dict(), f"{env_name}.pth")
